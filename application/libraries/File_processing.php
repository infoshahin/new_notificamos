...
class MY_File_processing
{
    ...
    public function make_upload($fieldName, $path, $type, $file_name, $width = 0, $height = 0)
    {
        ...
        if (!$this->CI->upload->do_upload($fieldName))
            return array('status' => 0, 'error' => $this->CI->upload->display_errors());
        else
    ...
    public function image_upload($fieldName, $path, $size = '', $type = '', $file_name = '')
    {
        ...
        if ($size != '')
        {
            ...
            $allSize = array('800,600'); // default size
...
// check the is set or not
       ...
       // custom image uploading configuration
    public function file_upload($fieldName, $path, $type = '', $file_name = '')
    {
    // check the is set or not
        if ($type == '')
            $type = 'pdf';
